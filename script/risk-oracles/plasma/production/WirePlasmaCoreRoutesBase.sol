...
/// @dev    Structural mirror of `WireEthereumCoreRoutesBase`; see it for the full reasoning on the
///         batch shape, the throttle pairing and the name derivation. **Broadcasts nothing**: by
///         phase 3 the safe has accepted the Router, so each phase prints its batch and the safe
///         executes it.
abstract contract WirePlasmaCoreRoutesBase is Script {
    ...
    function agentCalls(Context memory ctx) public pure returns (SafeTx.Call[] memory calls) {
        ...
        // The discount route is the only one the Router can rate limit meaningfully: its payload
        // is a 32-byte scalar, so both the min delay and the step cap apply.
        calls[0] = SafeTx.call(
        ...
        // The risk-params route publishes tuples, which the Router cannot decode, so the step
        // guard is expressed as off rather than as a number that would silently never apply.
        // Freezing this route means `setRouteEnabled(false)`.
        calls[2] = SafeTx.call(
    ...
    /// @dev    This is the last gate. After phase 3 the stack is live, and every way the payload
    ///         can be wrong fails silently rather than loudly, because the Router catches AgentHub
    ///         failures by design so a bad injection cannot roll back a good publish.
    function requireTheAipLanded(Context memory ctx) public view {
        ...
        // A fresh agent id inherits no default range config, and the module reads a missing one as
        // a zero bound, so an unset range is not a loose stack, it is a dead one.
    ...
    /// @dev Five facts per agent, all of them the payload's to get right and none of them visible
        ...
        view
    {
        ...
        // The agents are the payload's to construct, so the empty `updateTypeSuffix` is an
        // assumption on our side until it is read back.
        require(
        ...
        // agent is rejected by the PendlePriceCapAdapter and the eMode agent by the
        // PoolConfigurator.
        require(
    ...
    ///         The raw name is NEVER the answer and `cast format-bytes32-string "<name>"` is always
    ///         wrong here. Derived rather than passed in because `addRoute` accepts any wrong name
    ///         and there is no setter to correct one.
